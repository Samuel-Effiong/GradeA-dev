...
| 3 Concurrency | **PASS** | 20 simultaneous requests x 10 rounds = exactly 1 grant per round; 20 mixed self-service/admin threads; signup racing an admin assignment; an orchestrated interleaving that defeats check-then-act. Real threads, real PostgreSQL, each closing its own connection, `is_alive()` asserted after join. **Reading this beside 7a's table:** 7a records its own Gate 3 as PARTIAL, "true concurrent activation race not yet run". That describes its harness's coverage, not a hole in this fix — its repeat abuse was sequential (12x), while the concurrent race is covered here, and the mutation battery kills the row-lock mutant (M08) through exactly these tests. |
| 4 Adversarial / Attack | **PASS (independently verified, core vectors and bypass battery)** | Independent replay by session 7a (red-team-billing), which did not write the fix, committed at `dcc031b` on `task/redteam-billing`: `docs/evidence/security_replay/billing/FREE_PLAN_GATE4.md`. HTTP-only exploits with real JWT against two isolated running apps, accounts and plans created through the real production write paths. **7 vectors, each proven to SUCCEED on `b744c9f` first (H5.2), all refused on the fix** — teacher self-grants BETA on both routes; school admin grants a non-BETA free plan; inactive free plan; 12x repeat (pre-fix: 12/12 accepted, 13 subscriptions and 13 buckets; post-fix: 12x403); plan enumeration (pre-fix: every plan visible to a plain teacher; post-fix: PRO/STANDARD/STANDARD_ANNUAL only). It also probed superadmin admin-assign idempotency 5x per route: first 201 then 400x4, one active subscription, no duplicates — recorded as NOT a finding. I verified its four log checksums against the committed files myself, and its verdict on `d883187` carries to `a520422` because no production file differs between them (`git diff --name-only d883187 a520422` is tests and docs only). **Bypass battery (7a, `0b5e4eb`, run against `a520422` exactly — the log header names the full SHA):** field aliases 403; extra-field injection 403; form-encoded/browsable-API create 403; trailing slash 404; PATCH 403; select-plan with an allow-listed, active, Stripe-configured plan priced 0: clean 400 "no price configured" before any Stripe call (pre-fix it reached `StripeCheckoutService` and failed only on the attacker's fake price id — "reaches Stripe", not a demonstrated free grant). Subscriptions and credits unchanged after every vector. Aliases, trailing slash and PATCH were never viable pre-fix and are recorded as such. **Checksum reconciliation (resolved):** at `0b5e4eb` the two bypass logs did not match 7a's manifest. Cause, per 7a: its script printed trailing spaces, and the pre-commit trailing-whitespace hook rewrote both logs after they were hashed. Fixed at `ce253f8`. Verified by me, not taken on report: at `ce253f8` all six logs match the manifest, and both bypass logs are byte-identical to `0b5e4eb` (`git diff --quiet 0b5e4eb ce253f8 -- <log>`), so only the recorded numbers changed, never the evidence. The fixture step's "HTTP 400" was a duplicate-create against an existing plan; select-plan still ran against a genuine $0 plan. The traceback in the pre-fix 500 body is DEBUG-gated (`users/renderers.py`: `if exc and settings.DEBUG:`), so it is not an information-disclosure finding. **Cross-harness agreement:** the two independently written harnesses agree on the pre-fix tree, including an odd-shaped detail rather than only the headline — pre-fix, a school admin could NOT take BETA (the teacher-only guard stopped it) but COULD take a non-BETA free plan. A shared wrong assumption would not reproduce that asymmetry. |
...
not here. Gate 4 is independently verified in full (core vectors and bypass battery). Gate 8 is completed on QA after landing, per
...
4. **Which gates passed.** 2, 3, 4, 5, 6, 7, 9, and 1 for this branch's scope.
5. **Which gates remain incomplete.** Gate
