...
| **`dashboard`** | Four role-scoped analytics surfaces, at-risk detection, rigor roll-ups, weekly digests, inactivity/at-risk alerting, credit-billed AI chat over dashboard data | `views.py`, `services.py`, `tasks.py`, `risk.py`, `rigor.py`, `models.py`, `throttling.py` |
...
8. ~~One empty app remains installed.~~ **Resolved 2026-09-15.**
   `ocr_processor` contained nothing but scaffolding comments and was
   removed after verifying it had no models, migrations, content types,
   permissions, URL routes, beat tasks or importers — the same check
   applied to the equally empty `grading` app, deleted 2026-09-12. There is
   no dedicated OCR module: the vision model reads page images directly.
   `ai_processor/services.py::OCRService` is a small, still-live helper
   that only measures decoded image dimensions for the pre-flight cost
   estimate — it does not perform OCR, and no code path claims it does.
...
| # | Item |
| --- | --- |
| X1 | ~~App `ocr_processor` — empty, still in `INSTALLED_APPS`~~ — removed 2026-09-15 (`grading`, originally listed here too, was deleted 2026-09-12) |
