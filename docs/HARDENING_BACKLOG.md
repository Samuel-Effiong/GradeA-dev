...
| H-42 | `send_user_activation_email` (`users/services.py`) routes school admins to `FRONTEND_DOMAIN` (the teacher app) alongside teachers, same as the now-fixed school-admin invitation email was doing — but the school-admin frontend is a genuinely separate app that refuses other roles | Medium — same defect class as the invitation-email fix, not yet confirmed reachable/harmful in practice | Proposed: whoever picks up school-admin onboarding polish next | **Not started.** Found by privacy-guard (2026-09-23) while fixing the school-admin invitation email's domain (see `_send_school_admin_invitation_email`, `classrooms/serializers.py`). Deliberately left unfixed here rather than changed alongside: this is a shared function used by every user type, there is a pre-existing test (`users/tests_activation_email_domain.py::test_school_admin_gets_frontend_domain`) that explicitly asserts today's `FRONTEND_DOMAIN` routing (possibly a deliberate decision predating the school-admin app's existence as a separate frontend, possibly just unnoticed), and its link shape (`/verify-email`) doesn't match the invitation flow's (`/register/school-admin`) — so a same-shaped domain swap wouldn't necessarily make this path correct on its own. Reachable via the VERIFY_EMAIL OTP-resend endpoint for a school admin who hasn't completed activation. Needs investigation (is this path even reachable for a license-invited school admin vs. only a self-registered one; what should the URL shape be) before a safe fix, not a quick swap.
