...
get_or_render() adds single-flight on top of that cache, because the
cache alone does nothing for the moment it matters most. Measured: 30
simultaneous requests for one *uncached* assignment produced 30 identical
Chromium renders - every one of them a full miss, because none had
finished storing a result yet. That is exactly the shape of a teacher
publishing an assignment and a class opening it at once. With
single-flight the first caller renders and the rest wait for its result.
"""
...
class _Flight:
    """One in-progress render that other callers can wait on."""
...
# key -> _Flight, for renders currently running in THIS process.
_inflight: "dict[str, _Flight]" = {}
_inflight_lock = threading.Lock()


def _follower_timeout() -> float:
    """
    How long a waiting caller gives the render already in progress before
    giving up and rendering for itself.

    Must exceed a normal render (assignments.pdf_renderer's own bound is
    30s plus queue slack) or followers would routinely bail out and
    re-render exactly what they were waiting for - reintroducing the
    stampede this exists to prevent.
    """
    return getattr(settings, "ASSIGNMENT_PDF_SINGLEFLIGHT_TIMEOUT_SECONDS", 60.0)


def get_or_render(assignment, view_type: str, render_fn):
    """
    Return this assignment's PDF, rendering it at most once per process
    even when many callers ask for the same one simultaneously.

    The first caller for a given key renders and stores; callers that
    arrive while that is running wait for its result instead of starting
    their own identical render.

    Scope is deliberately per-process, not cluster-wide. With N gunicorn
    workers a simultaneous burst costs at most N renders instead of one
    per request - measured, 30 concurrent requests dropped from 30 renders
    to 1 in a single process. Making it exactly 1 cluster-wide would need
    a distributed lock, which brings failure modes (expiry, a holder that
    dies mid-render) far worse than the duplicate work it would save.

    If the in-progress render fails, the waiters are given the same error
    rather than each retrying: the renderer already retries internally on
    a dead browser, so a failure that reaches here is one a retry storm
    would only repeat.
    """
    cached = get_cached_pdf(assignment, view_type)
    if cached is not None:
        return cached

    key = build_cache_key(assignment, view_type)
    ...
    if not leader:
        if not flight.done.wait(timeout=_follower_timeout()):
            # The leader is pathologically slow (or died in a way that
            # skipped its cleanup). Rendering for ourselves is slower than
            # waiting, but it always terminates.
            logger.warning(
                "[PDF] timed out waiting for an in-progress render of "
                "assignment %s (%s view); rendering separately.",
                assignment.id,
                view_type,
            )
            return render_fn()
    ...
    try:
        ...
        flight.result = pdf_bytes
        # Stored before the waiters are released, so anyone arriving just
        # after this flight is cleaned up gets a cache hit rather than
        # starting another render of the same thing.
